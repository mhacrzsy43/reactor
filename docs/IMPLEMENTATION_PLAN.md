...
| M8.10 | 🟡 进行中（必做） | M8.10A 已完成：Android/iOS 画面与 UI 树同步、Selector Inspector、测量隔离和 Android Emulator 桌面门禁通过；下一步 M8.10B 审查/录制双模式、逐页交互、输入与实时 Flow 编辑 |
...
- **M8.10B：操作录制、编辑与回放**：提供互不混淆的“审查模式”和“录制/交互模式”；审查模式点击只选择 Selector，录制模式点击后真实操作设备、自动刷新到下一页面并把语义化步骤加入 Flow，让用户逐页继续选择。输入、滚动、返回和等待同样转换成 Reactor Flow；同步展示步骤、Flow JSON 和实际 Maestro YAML；允许插入、删除、重排、撤销、修改 Selector，并在真实设备上逐步或整体回放。坐标只能作为显式标记的脆弱降级，删除、支付、授权、提交等敏感控件必须二次确认。
  ...
  - **动态与敏感输入**：支持普通常量、测试数据变量和本机 Secret 引用。密码、验证码、Token 等默认遮罩且不得以明文写入 Flow、日志、截图说明或 AI 上下文；回放时从本机安全存储/CI Secret 注入。验证码等一次性值允许交互试跑，但必须标记为不可直接复现。
